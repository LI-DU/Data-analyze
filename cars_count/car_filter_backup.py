...
if __name__ == "__main__":
    with open('./新能源汽车推荐目录.txt', 'r', encoding='gbk') as fr:
        content = fr.read()
    ...
    cars = list_content[1:]
    cars[0:0] = [tmp[-1]]
    car_type = set([w.strip() for w in open('./car_type.txt',encoding='utf-8').readlines()])
    car_info = set([w.strip() for w in open('./car_info.txt',encoding='utf-8').readlines()])
    jieba.load_userdict('./car_info.txt')
    jieba.load_userdict('./car_type.txt')
    ...
    with open('./新能源汽车推荐目录_筛选.txt','w',encoding='utf-8') as fw:
        txt = '\n\n'.join(title)
        fw.write(txt)
        fw.write('\n\n')
