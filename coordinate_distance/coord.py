...
def mysql_query():
    ...
    # sql中查询在此坐标5km范围内的充电站坐标
        ...
        select_cord = """select cast(longitude AS CHAR ),cast(latitude AS CHAR ) from t_charging_station where latitude > %s and latitude < %s and longitude > %s and longitude < %s"""%(lat2,lat1,lng1,lng2)   #(lat_lng[2],lat_lng[0],lat_lng[1],lat_lng[3])
